"""
回填脚本 V3 测试版 - 只处理2024年1-3月
关键改进：按日期获取全市场数据，不使用当前股票列表
"""
import pandas as pd
import numpy as np
...
def process_index_data(index_name, index_code, start_date, end_date):
    ...
    print(f"      合并后记录总数: {len(df_all)} 条")
    print(f"      有换手率数据的记录: {with_turnover} 条 ({with_turnover/len(df_all)*100:.1f}%)")
...
def run_backfill_v3_test():
    """运行回填V3测试版 - 只处理2024年1-3月"""
    # 测试模式：只处理2024年1-3月
    START_DATE = '20240101'
    END_DATE = '20240331'

    print(f"=== 回填V3测试版 - 验证修复效果 ===")
    ...
    # 只测试一个指数
    test_indices = {'沪深300': '000300.SH'}

    for index_name, index_code in test_indices.items():
        print(f"\n====== 处理指数: {index_name} ======")
        ...
        print(f"\n计算结果:")
        print(df_final[['trade_date', 'total_count', 'pct_turnover_lt_3', 'pct_turnover_gt_5']].head(20))
        print(df_final[['trade_date', 'total_count', 'pct_turnover_lt_3', 'pct_turnover_gt_5']].tail(10))
