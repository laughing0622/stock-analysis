"""
回填脚本 V2 - 使用正确的全量查询方式
关键改进：daily_basic接口用ts_code=''按日期批量获取，而不是逐个股票查询
"""
import pandas as pd
import numpy as np
...
def process_index_data(index_name, index_code, start_date, end_date, all_stocks_set=None):
    """处理单个指数的数据回填"""
...
def run_backfill_v2():
    ...
    print(f"=== 回填V2 - 使用全量查询方式 ===")
    print(f"日期范围: {START_DATE} ~ {END_DATE}")
    print(f"换手率获取方式: ts_code='' 按日期批量获取")
    ...
    for index_name, index_code in INDEX_MAP.items():
        ...
        # 处理数据
        df_breadth = process_index_data(index_name, index_code, START_DATE, END_DATE, all_stocks_set)
